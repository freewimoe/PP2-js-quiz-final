...
// Complete Quiz Data with 4 questions each and explanations
const QUIZ_DATA = {
  baroque: {
...
const startQuiz = (theme) => {
  try {
    ...
    setTimeout(() => {
      ...
      setTheme(theme);
      if (quizTitle) quizTitle.textContent = QUIZ_DATA[theme].title;
...
const showFeedback = (isCorrect, explanation) => {
  if (!feedbackElement || !feedbackIcon || !feedbackText) return;
...
const showResults = () => {
  try {
    ...
    // Set score message
    if (scoreText) scoreText.textContent = getScoreMessage(score, currentQuiz.length);
